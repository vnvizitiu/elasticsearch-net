...
namespace Nest
{
	...
	{
		Dictionary<IndexName, IDictionary<TypeName, TypeMapping>> Mappings { get; }
	...
	public class GetMappingResponse : ResponseBase, IGetMappingResponse
	{
		...
		internal GetMappingResponse(GetRootObjectMappingWrapping dict)
		{
			foreach (var index in dict)
			{
				...
				{
					this.Mappings.Add(index.Key, new Dictionary<TypeName, TypeMapping>());
		...
		public Dictionary<IndexName, IDictionary<TypeName, TypeMapping>> Mappings { get; internal set; } = new Dictionary<IndexName, IDictionary<TypeName, TypeMapping>>();
